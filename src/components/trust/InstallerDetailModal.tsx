...
export function InstallerDetailContent({ installer, onClose, embedded = false }: { installer: Installer, onClose?: () => void, embedded?: boolean }) {
    return (
        <div className={`bg-zinc-900 border border-white/10 w-full overflow-y-auto rounded-3xl shadow-2xl flex flex-col ${embedded ? 'h-full border-0 rounded-none shadow-none' : 'max-w-4xl max-h-[90vh]'}`}>
        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    >
                        <InstallerDetailContent installer={installer} onClose={onClose} />
                    </motion.div>
                </>
            )}
        </AnimatePresence>
